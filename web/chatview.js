...
function removeInteraction(interaction_id) {
    ...
    if (interaction.previousSibling) {
        /* if element was the most recently received message, make sure the
           last-message property is given away to the previous sibling */
        if (interaction.classList.contains("last-message")) {
            interaction.previousSibling.classList.add("last-message")
        }
...
function addOrUpdateMessage(message_object, new_message, insert_after = true, messages_div) {
    ...
    if (new_message) {
        ...
        if (insert_after) {
            if (messages_div.lastChild) {
                messages_div.lastChild.classList.remove("last-message")
            }

            if (previousMessage) {
                var prev_message_direction = previousMessage.classList.contains("message_out") ? "out" : "in"
                var sequence_break = prev_message_direction != message_direction
                if (!sequence_break) {
                    message_div.classList.add("last_of_sequence")
                    previousMessage.classList.remove("last_of_sequence")
                    previousMessage.classList.remove("single_message")
                    if (!previousMessage.classList.contains("first_of_sequence")) {
                        previousMessage.classList.add("middle_of_sequence")
                    }
                } else {
                    message_div.classList.add("first_of_sequence")
            ...
            window.jsbridge.debug("append" + message_div.name)
            messages_div.classList.add("last-message")
            ...
            window.jsbridge.debug("prepend: " + message_div.getAttribute('id'))
            var nextMessage = messages_div.firstChild
            messages_div.prepend(message_div)
            if (nextMessage) {
                window.jsbridge.debug("nextMessage: " + nextMessage.getAttribute('id'))
                var next_message_direction = nextMessage.classList.contains("message_out") ? "out" : "in"
                var sequence_break = next_message_direction != message_direction
                if (!sequence_break) {
                    message_div.classList.add("first_of_sequence")
                    nextMessage.classList.remove("first_of_sequence")
                    nextMessage.classList.remove("single_message")
                    if (!nextMessage.classList.contains("last_of_sequence")) {
                        nextMessage.classList.add("middle_of_sequence")
                    }
                } else {
                    message_div.classList.add("last_of_sequence")
                    message_div.classList.add("single_message")
...
function printHistoryPart(messages_div, fixedAt)
{
    ...
    var nextMessage = lastMessage.nextElementSibling
    if (!nextMessage) {
        var nextMessagesBlock = block_wrapper.nextElementSibling
        if (nextMessagesBlock) {
            window.jsbridge.debug(nextMessagesBlock.getAttribute('class'))
            var nextMessage = nextMessagesBlock.firstChild
            if (nextMessage) {
                var next_message_direction = nextMessage.classList.contains("message_out") ? "out" : "in"
                var last_message_direction = lastMessage.classList.contains("message_out") ? "out" : "in"
                var sequence_break = next_message_direction != last_message_direction
                if (!sequence_break) {
                    lastMessage.classList.add("first_of_sequence")
                    nextMessage.classList.remove("first_of_sequence")
                    nextMessage.classList.remove("single_message")
                    if (!nextMessage.classList.contains("last_of_sequence")) {
                        nextMessage.classList.add("middle_of_sequence")
                    ...
                    lastMessage.classList.add("last_of_sequence")
                    lastMessage.classList.remove("middle_of_sequence")
                }
            }
        }
    }
    ...
    if (absoluteLastMessage) {
        absoluteLastMessage.classList.add("last-message")
    }
