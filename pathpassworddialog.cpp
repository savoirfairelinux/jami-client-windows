...
PathPasswordDialog::on_pathButtonEdit_clicked()
{
    QString filePath;
    if (exportMode) {
        filePath = QFileDialog::getSaveFileName(this, tr("Save File"), QString(), tr("Ring archive files (*.ring)"));
    } else {
        filePath = QFileDialog::getOpenFileName(this, tr("Open File"), QString(), tr("Ring archive files (*.ring)"));
    }
...
void
PathPasswordDialog::on_passwordLineEdit_textChanged(const QString &arg1)
{
    password_ = arg1;
}
