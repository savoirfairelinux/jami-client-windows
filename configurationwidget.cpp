...
void ConfigurationWidget::on_deleteAccountButton_clicked()
{
    Account* account = accountModel_->getAccountByModelIndex(ui->accountView->currentIndex());
    if (account != accountModel_->ip2ip())
        accountModel_->remove(account);
}

void ConfigurationWidget::on_addAccountButton_clicked()
{
    accountModel_->add("New Account", ui->accountTypeBox->model()->index(ui->accountTypeBox->currentIndex(), 0));
    accountModel_->save();
}
