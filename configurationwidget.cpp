...
ConfigurationWidget::initLrcConnections()
{
    ...
    // profile
    const auto currentAccountIndex = accountModel_->selectionModel()->currentIndex();
    ui->profileNameEdit->setText(currentAccountIndex.data(static_cast<int>(Account::Role::Alias)).value<QString>());
...
//        currentAccount_->setDisplayName(name);
}
