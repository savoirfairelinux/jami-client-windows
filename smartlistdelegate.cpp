                        ...
                        ) const
{
	...
	if ( currentRow == rowHighlighted )
    {
		painter->fillRect( opt.rect, QColor( 242, 242, 242 ) );

		emit MonSignal( opt.rect );
    ...
    painter->drawLine( rect.left()+20, rect.bottom(), rect.right()-20
                                                      , rect.bottom() );
    ...
    if (index.column() == 0) 
    {
        ...
        if ( state.isValid() && RecentModel::instance().getActiveCall( RecentModel::instance().peopleProxy()->mapToSource(index) ) ) 
        {
            ...
            painter->drawText( QRect(16+rect.left()+dx+sizeImage_, // NICOO DEFINIR UN QRECT AVEC LES AUTRES
        ...
        else
        {
            ...
            if ( lastUsed.isValid() )
            {
                ...
                painter->drawText( QRect(16+rect.left()+dx+sizeImage_, // NICOO DEFINIR UN QRECT AVEC LES AUTRES
                                   rect.top() + rect.height()/2,
                                   rect.width(), rect.height()/2),
                                   Qt::AlignTop | Qt::AlignLeft, lastUsed.toString()); // NICOO CONTACTE DERNIERE FOIS            
            }
        }
