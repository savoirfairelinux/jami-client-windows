...
ServiceBase::serviceStart(DWORD argc, PWSTR * argv)
{
    try {
        ...
        // Log the error.
        writeEventLogEntry(serviceName_, dwError, EVENTLOG_ERROR_TYPE);
...
void
ServiceBase::writeEventLogEntry(PWSTR serviceName, PWSTR message, WORD type)
{
...
BOOL
ServiceBase::findProcessHandle(PWSTR currentServiceName, std::wstring& processName, HANDLE& hProcess)
{
    HANDLE hProcessSnap;
    PROCESSENTRY32 pe32;
    ...
    if (hProcessSnap == INVALID_HANDLE_VALUE) {
        writeEventLogEntry(currentServiceName, L"Error when taking process snapshot", EVENTLOG_ERROR_TYPE);
        return FALSE;
    }
    ...
    if (!Process32First(hProcessSnap, &pe32)) {
        writeEventLogEntry(currentServiceName, L"Error when finding the first process", EVENTLOG_ERROR_TYPE);
    ...
    do {
        ...
        if (hProcess == NULL) {
            writeEventLogEntry(currentServiceName, L"Error when openning process", EVENTLOG_ERROR_TYPE);
        }
