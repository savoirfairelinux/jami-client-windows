...
JamiProcessMonitor::serviceWorkerThread(void* param)
{
    ...
    while (!currentInstance->stopping_) {
        ...
        if (findProcessHandle(currentInstance->getServiceName(), monitor_process_name, hProcess)) {
            ...
            // Wait until Jami exits.
            auto ret = WaitForSingleObject(hProcess, INFINITE);
            if (ret == WAIT_FAILED) {
                writeEventLogEntry(currentInstance->getServiceName(), ServiceBase::GetLastErrorAsPWSTR(), EVENTLOG_ERROR_TYPE);
                break;
            }
            ...
            writeEventLogEntry(currentInstance->getServiceName(), PWSTR((std::wstring(L"Exit Code is ") + std::to_wstring(lpExitCode)).c_str()), EVENTLOG_INFORMATION_TYPE);
