...
ImDelegate::formatMsg(const QModelIndex& index, QString& msg) const
{
    ...
    if (settings.value(SettingsKey::imShowAuthor).toBool()) {
        meta << index.data(
                    static_cast<int>(LRCMedia::TextRecording::Role::AuthorDisplayname)).toString();
    }
    if (settings.value(SettingsKey::imShowDate).toBool()) {
        auto timeStamp = index.data(
                    static_cast<int>(LRCMedia::TextRecording::Role::Timestamp)).value<uint>();
                  ...
                  const QModelIndex& index) const
{
    ...
    if (index.isValid()) {
        auto msg = index.data(static_cast<int>(LRCMedia::TextRecording::Role::FormattedHtml)).toString();
        opt.text.clear();
        QStyle* style = opt.widget ? opt.widget->style() : QApplication::style();

        auto dir = index.data(static_cast<int>(LRCMedia::TextRecording::Role::Direction))
                .value<LRCMedia::Media::Direction>() == LRCMedia::Media::Direction::IN
                     ...
                     const QModelIndex& index) const
{
    ...
    QString msg = index.data(static_cast<int>(LRCMedia::TextRecording::Role::FormattedHtml)).toString();

    auto dir = index.data(
                static_cast<int>(LRCMedia::TextRecording::Role::Direction))
            .value<LRCMedia::Media::Direction>() == LRCMedia::Media::Direction::IN
            ? Qt::AlignLeft : Qt::AlignRight;
