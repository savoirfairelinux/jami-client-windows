                  ...
                  const QModelIndex& index) const
{
    ...
    QRect textRect = Utils::getBoundingRect(dir, opt, document);
    ...
    // HACK: only draw text if not hovering, when selectabletextwidget is displayed
    auto hasIndexWidget = index
        .data(static_cast<int>(MessageModel::Role::HasIndexWidget))
        .value<bool>();
    if ((!hasIndexWidget || isGenerated)) {
                     ...
                     const QModelIndex& index) const
{
    ...
    auto isGenerated = Utils::isInteractionGenerated(
        static_cast<lrc::api::interaction::Type>(index
            .data(static_cast<int>(MessageModel::Role::Type))
            .value<int>())
    );
    auto dir = isGenerated ? Qt::AlignHCenter : (isOutgoing ? Qt::AlignRight : Qt::AlignLeft);
    ...
    QRect boundingRect = Utils::getBoundingRect(dir, opt, document);
    ...
    //qDebug() << "sizeHint boundingRect:" << boundingRect;
