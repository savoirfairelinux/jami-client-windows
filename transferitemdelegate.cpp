...
void TransferItemDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
    ...
    if (isInProgress) {
        ...
        auto cancelRect = QRect(opt.rect.left() + progressBarOption.rect.width() + 5,
        ...
        painter->drawPixmap(cancelRect, cancelPixmap.scaled(QSize(24,24), Qt::KeepAspectRatio, Qt::SmoothTransformation));

        painter->restore();

    } else {

        auto acceptButtonRect = QRect(opt.rect.left() + opt.rect.width() / 2 - 40,
        ...
        painter->drawPixmap(acceptButtonRect, acceptPixmap.scaled(QSize(32,32), Qt::KeepAspectRatio, Qt::SmoothTransformation));

        painter->restore();


        painter->save();
        auto refuseRect = QRect(acceptButtonRect.left() + acceptButtonRect.width() + 5,
        ...
        painter->drawPixmap(refuseRect, refusePixmap.scaled(QSize(32,32), Qt::KeepAspectRatio, Qt::SmoothTransformation));
                                       ...
                                       const QModelIndex& index)
{
    ...
    auto acceptButtonRect = QRect(option.rect.left() + option.rect.width() / 2  - 40,
                                  option.rect.top()+ option.rect.height() / 2 - 20,
                                  40,
                                  40);
    auto refuseButtonRect = QRect(acceptButtonRect.left() + acceptButtonRect.width() + 5,
                                  acceptButtonRect.top(),
                                  40,
                                  40);
    auto progressBarRect = QRect(option.rect.left(),
                                 ...
                                 30);
    auto cancelButtonRect = QRect(option.rect.left() + progressBarRect.width() + 5,
                                  progressBarRect.top(),
                                  30,
                                  30);
