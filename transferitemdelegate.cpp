                            ...
                            const QModelIndex& index) const
{
    ...
    auto transfer = FileTransferModel::instance().getTransferByModelIndex(index);
    auto isInProgress = (transfer->status() >= 100) && (transfer->status() < 400);
    auto isError = (transfer->status() >= 400);

    qDebug() << transfer->status();

    auto fileName = transfer->filename();
    auto senderAccount = transfer->contactMethod() ? transfer->contactMethod()->primaryName() : "Unknown" ;
    ...
    if (isInProgress || isError) {
        ...
        if (true) {
            progressBarOption.text = transfer->dataTransferCodeToString();
            progressBarOption.maximum = 0;
        } else {
            int progress = transfer->progress();
                                  ...
                                  const QModelIndex& index)
{
    ...
    auto isInProgress = (transfer->status() >= 200 && transfer->status() <= 400);
    ...
    if(event->type() == QEvent::MouseButtonPress) {
        ...
        if (isAcceptButton && !isInProgress) {
            acceptState_ = QStyle::State_Raised;
            //TODO: Change that to a user choice
            transfer->accept("C:/temp");
        }
        else if (isRefuseButton && !isInProgress) {
            refuseState_ = QStyle::State_Raised;
            transfer->cancel();
