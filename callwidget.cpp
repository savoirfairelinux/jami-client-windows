                             ...
                             const lrc::api::conversation::Info& convInfo)
{
    ...
    // insert if needed, and change current video widget
    currentVideoView_ = videoViewMap_[convInfo.callId];
    if (currentVideoView_ ==  nullptr) {
        currentVideoView_ = new VideoView(convInfo, this);
        videoViewMap_[convInfo.callId] = currentVideoView_;
        ui->videoPage->layout()->addWidget(currentVideoView_);
    }

    currentVideoView_->show();
    currentVideoView_->setFocus();

    connect(currentVideoView_, &VideoView::setChatVisibility,
        [this](bool visible) {
            if (visible) {
                ui->messagesWidget->show();
            } else {
    ...
    connect(currentVideoView_, &VideoView::toggleFullScreenClicked,
            this, &CallWidget::slotToggleFullScreenClicked);

    connect(currentVideoView_, &VideoView::closing,
...
{
    ui->videoPage->layout()->removeWidget(videoViewMap_[id]);
    videoViewMap_.erase(id);
...
CallWidget::connectConversationModel()
{
    ...
    newInteractionConnection_ = QObject::connect(
        ...
        [this](const std::string& convUid, uint64_t interactionId, const lrc::api::interaction::Info& interaction) {
            ...
            if (convUid == LRCInstance::getSelectedConvUid()) {
                if (currentVideoView_) {
                    currentVideoView_->simulateShowChatview(true);
                }
            }
        }
    );
