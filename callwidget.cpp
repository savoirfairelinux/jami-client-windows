...
void
CallWidget::triggerDeleteContactDialog(ContactMethod *cm)
{
...
void
CallWidget::addPerson(ContactMethod* cm)
{
    // only one collection on windows
    auto personCollection = PersonModel::instance().collections().at(0);
    auto newPerson = new Person();
    newPerson->setFormattedName(cm->bestName());
    Person::ContactMethods cms;
    cms.append(cm);
    newPerson->setContactMethods(cms);
    cm->setPerson(newPerson);
...
CallWidget::setupSmartListMenu(const QPoint& pos)
{
    ...
    if (!contactMethod->contact() || contactMethod->contact()->isPlaceHolder()) {
        ...
        connect(addExisting, &QAction::triggered, [this, contactMethod]() {
        ...
        connect(removeExisting, &QAction::triggered, [this, contactMethod]() {
            triggerDeleteContactDialog(contactMethod);
        });
    }
