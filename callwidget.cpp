...
void CallWidget::findRingAccount() {
    ...
    for (int i = 0; i < a_count; ++i) {
        ...
        if ((Account::Protocol)protocol.toUInt() == Account::Protocol::RING) {
            QVariant username = idx.data(static_cast<int>(Account::Role::Username));
            ui->ringIdLabel->setText(ui->ringIdLabel->text() + " " + username.toString());
            found = true;
            return;
        }
    }
...
void CallWidget::callStateChanged(Call* call, Call::State previousState) {
    ...
    if (call->state() == Call::State::OVER) {
        ...
        ui->videoWidget->show();
        connect(actualCall_, SIGNAL(isOver(Call*)), this, SLOT(callStateChanged(Call*,Call::State::OVER)));
