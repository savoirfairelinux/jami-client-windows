...
CallWidget::smartListSelectionChanged(const QItemSelection  &selected, const QItemSelection  &deselected)
{
    ...
    qDebug() << "index row: " << selectedIndex.row();
    selectConversation(selectedIndex);

    auto selectedConvUid = LRCInstance::getSelectedConvUid();
    if (!previousConvUid_.compare(selectedConvUid)) {
        return;
    }
    previousConvUid_.assign(selectedConvUid);
    auto currentConversationModel = LRCInstance::getCurrentConversationModel();
    auto callModel = LRCInstance::getCurrentCallModel();
    auto conversation = Utils::getConversationFromUid(selectedConvUid, *currentConversationModel);
    Call* thisCall = nullptr;
    const auto item = currentConversationModel->filteredConversation(selectedIndex.row());
    if (callModel->hasCall(conversation->callId)) {
        ui->stackedWidget->setCurrentWidget(ui->videoPage);
    } else {
        showIMOutOfCall(selectedIndex);
    }
}
...
CallWidget::on_ringContactLineEdit_returnPressed()
{
    ...
    auto firstContact = conversations.at(0).participants.at(0);
    auto isTemp = (convModel->owner.contactModel->getContact(firstContact).profileInfo.type == lrc::api::profile::Type::TEMPORARY);
    qDebug() << "istemp: " << isTemp;
}
