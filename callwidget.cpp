    ...
    imDelegate_(new ImDelegate())
{
    ...
    inviteItemDelegate_ = new InviteItemDelegate();
...
CallWidget::setSelectedAccount(const std::string& accountId)
{
    ...
    updateConversationFilterWidget();
...
void CallWidget::updateConversationFilterWidget()
{
    auto invites = LRCInstance::getCurrentAccountInfo().contactModel->pendingRequestCount();
    ui->conversationFilterWidget->setVisible(invites);
}
...
CallWidget::updateSmartList()
{
    ...
    if (currentTypeFilter_ == lrc::api::profile::Type::RING) {
        qDebug() << "foo";
        ui->smartList->setItemDelegate(conversationItemDelegate_);
    } else if (currentTypeFilter_ == lrc::api::profile::Type::PENDING) {
        qDebug() << "poof";
        ui->smartList->setItemDelegate(inviteItemDelegate_);
    }
