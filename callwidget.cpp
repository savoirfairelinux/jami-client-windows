    ...
    imDelegate_(new ImDelegate())
{
    ...
    try {
        ...
        smartListModel_ = std::make_unique<SmartListModel>(getCurrentAccountInfo(), parent);
...
CallWidget::on_sendIMButton_clicked()
{
    ...
    ui->imMessageEdit->clear();
    auto convUid = selectedConvUid_;
    try {
        getCurrentConversationModel()->sendMessage(convUid, msg.toStdString());
    } catch (...) {
...
{
    auto currentConversationModel = getCurrentAccountInfo().conversationModel.get();
    ...
    modelSortedConnection_ = QObject::connect(
        ...
        [this]() {
            ...
            auto selectedConversation = Utils::getConversationFromUid(selectedConvUid_,
                                                                      *getCurrentConversationModel());
            auto contactURI = QString::fromStdString((*selectedConversation).participants[0]);
...
CallWidget::updateConversationView(const std::string& convUid)
{
    ...
    auto& currentAccountInfo = getCurrentAccountInfo();
...
CallWidget::getCurrentAccountInfo()
{
    auto currentAccountId = getSelectedAccount()->id().toStdString();
    return LRCInstance::accountModel().getAccountInfo(currentAccountId);
}

lrc::api::ConversationModel*
CallWidget::getCurrentConversationModel()
{
...
{
    auto currentConversationModel = getCurrentConversationModel();
