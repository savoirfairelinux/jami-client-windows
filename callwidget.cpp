    ...
    imDelegate_(new ImDelegate())
{
    ...
    connect(ui->sendContactRequestWidget, &SendContactRequestWidget::sendCRclicked, [=] {Utils::setStackWidget(ui->stackedWidget, ui->messagingPage); });
    ...
    // select last used account(first in list)
    auto accountList = LRCInstance::accountModel().getAccountList();
    if (!accountList.empty()) {
        setSelectedAccount(accountList.at(0));
    }
