...
SmartList::eventFilter(QObject* watched, QEvent* event)
{
    ...
    // this one discard some bad user behavior...
    else if (qobject_cast<QScrollBar*>(watched) && event->type() == QEvent::Wheel
...
SmartList::update(const QModelIndex& index)
{
    ...
    smartListDelegate_->setHoveredRow(index.isValid() ? index : QModelIndex());
    repaint();
}
