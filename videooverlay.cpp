...
VideoOverlay::on_transferButton_toggled(bool checked)
{
    ...
    Utils::oneShotConnect(contactPicker_, &ContactPicker::willClose,
        [this](QMouseEvent *event) {
            ...
            auto globalClickPos = event->globalPos();
            auto grelativeCursorPos = ui->transferCallButton->mapFromGlobal(globalClickPos);
            if (!ui->transferCallButton->rect().contains(grelativeCursorPos)) {
