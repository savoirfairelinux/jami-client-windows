...
LinkDevWidget::setGeneratingPage()
{
    ...
    timeout_->setInterval(exportTimeout);
    timeout_->setSingleShot(true);
    connect(timeout_, &QTimer::timeout, this, [&](){setExportPage(std::string(), lrc::api::account::ExportOnRingStatus::NETWORK_ERROR,
        std::string());  });
    timeout_->start();
...
LinkDevWidget::setExportPage(const std::string& accountId, lrc::api::account::ExportOnRingStatus status, const std::string& pin)
{
    ...
    default:
        disconnect(&LRCInstance::accountModel());
