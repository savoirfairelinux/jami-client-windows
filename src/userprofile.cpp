...
UserProfile::UserProfile(lrc::api::conversation::Info convInfo, QWidget* parent)
{
    ...
    userProfileWidget_->rootObject()->setProperty("userImageSource", userImageSource);
    userProfileWidget_->rootObject()->setProperty("qrImageSource", userQrImageSource);

    userProfileWidget_->rootObject()->setProperty("userNameText", name.isEmpty() ? registeredName : name);
