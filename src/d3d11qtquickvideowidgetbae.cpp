...
D3D11Renderer::updateTextures(AVFrame* frame)
{
    ...
    if (frame->linesize[0] && frame->linesize[1] && frame->linesize[2]) {
        QMutexLocker lock(&mutexUpdateTextures_);
        ...
        if (AVPixelFormat(frame->format) == AVPixelFormat::AV_PIX_FMT_YUV420P) {
            ...
            initializeTexture(texU_, shaderResourceViewU_, updateOrRecreate, frame->width / 2, frame->height / 2, texUIndex_, DXGI_FORMAT_R8_UNORM, resourceDataTexU);

            resourceDataTexV.pSysMem = frame->data[2];
            resourceDataTexV.SysMemPitch = frame->linesize[2];
            initializeTexture(texV_, shaderResourceViewV_, updateOrRecreate, frame->width / 2, frame->height / 2, texVIndex_, DXGI_FORMAT_R8_UNORM, resourceDataTexV);
            result = true;
            ...
            resourceDataTexU.pSysMem = frame->data[1];
            resourceDataTexU.SysMemPitch = frame->linesize[1];
            initializeTexture(texU_, shaderResourceViewU_, updateOrRecreate, frame->width / 2, frame->height, texUIndex_, DXGI_FORMAT_R8_UNORM, resourceDataTexU);

            resourceDataTexV.pSysMem = frame->data[2];
            resourceDataTexV.SysMemPitch = frame->linesize[2];
            initializeTexture(texV_, shaderResourceViewV_, updateOrRecreate, frame->width / 2, frame->height, texVIndex_, DXGI_FORMAT_R8_UNORM, resourceDataTexV);
            result = true;
            ...
            resourceDataTexU.pSysMem = frame->data[1];
            resourceDataTexU.SysMemPitch = frame->linesize[1];
            initializeTexture(texU_, shaderResourceViewU_, updateOrRecreate, frame->width, frame->height, texUIndex_, DXGI_FORMAT_R8_UNORM, resourceDataTexU);

            resourceDataTexV.pSysMem = frame->data[2];
            resourceDataTexV.SysMemPitch = frame->linesize[2];
            initializeTexture(texV_, shaderResourceViewV_, updateOrRecreate, frame->width, frame->height, texVIndex_, DXGI_FORMAT_R8_UNORM, resourceDataTexV);
            result = true;
        }
    }
    ...
    if (frame->linesize[0] && frame->linesize[1]) {
        ...
        if (AVPixelFormat(frame->format) == AVPixelFormat::AV_PIX_FMT_NV12) {
            ...
            initializeTexture(texUV_NV12_, shaderResourceViewNV12_, updateOrRecreate, frame->width, frame->height / 2, texUV_NV12_Index_, DXGI_FORMAT_R8G8_UNORM, resourceDataTexUV);
...
D3D11Renderer::updateTextureFromCUDA(AVFrame* frame, bool isTextureToBeUpdatedOrRecreated)
{
    ...
    if ((AVPixelFormat)avHWFramesContext->sw_format == AVPixelFormat::AV_PIX_FMT_NV12) {
        ...
        initializeTextureWithCUDA(texUV_NV12_, shaderResourceViewNV12_, isTextureToBeUpdatedOrRecreated, frame->width, frame->height / 2, frame->linesize[1],
            texUV_NV12_Index_, DXGI_FORMAT_R8G8_UNORM, pointerToDataBufferUV, cudaContext);
...
D3D11Renderer::mainPassRecordingStart()
{
    ...
    viewPort.Width = m_parantingItem->width();
    viewPort.Height = m_parantingItem->height();
    ...
    if (d3dDevContext_) {
...

        {
            ...
            // set related rendaer target views of related textures
            if (texY_ && texU_ && texV_) {
                initializeShaderResourceViewWithTexture(texY_, shaderResourceViewY_, texYIndex_);
                initializeShaderResourceViewWithTexture(texU_, shaderResourceViewU_, texUIndex_);
                initializeShaderResourceViewWithTexture(texV_, shaderResourceViewV_, texVIndex_);
            } else if (texY_ && texUV_NV12_) {
                initializeShaderResourceViewWithTexture(texY_, shaderResourceViewY_, texYIndex_);
                initializeShaderResourceViewWithTexture(texUV_NV12_, shaderResourceViewNV12_, texUV_NV12_Index_);
            }
...
{
    if ( !renderer_ ) {
        renderer_ = new D3D11Renderer(this);
    ...
    : D3D11QtQuickVideoWidgetBase()
{
    connect(LRCInstance::renderer(), &RenderManager::previewD3DFrameUpdated,
        [this]() {
            ...
            {
                QMutexLocker lock(&(renderer_->mutexUpdateTextures_));
...
void D3D11QtQuickVideoPreviewWidget::sync()
{
    if (renderer_) renderer_->constantBufferDataStruct_.boarderRadiusNormalizedShortSide = roundedCornerRadius_;
...
D3D11QtQuickVideoPreviewWidget::updatePreviewWindowSizeAccordingToAspectRatio()
{
    if ( aspectRatio > 1 ) {
        ...
        setHeight(currentHeight / aspectRatio);
    } else if( aspectRatio > 0 && aspectRatio < 1 ) {
        qreal currentWidth = width();
        setWidth(currentWidth * aspectRatio);
    }
}

D3D11QtQuickVideoDistantwWidget::D3D11QtQuickVideoDistantwWidget()
    : D3D11QtQuickVideoWidgetBase()
{
    connect(LRCInstance::renderer(), &RenderManager::previewD3DFrameUpdated,
        [this]() {
