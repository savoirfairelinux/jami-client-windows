...
class NewWizardViewQmlObjectHolder : public QObject
{
    ...
    Q_PROPERTY(QVariantMap inputPara READ getInputPara WRITE setInputPara NOTIFY inputParaChanged)

public:
    using AccountInfo = Info;

    enum class WizardMode { CREATE, IMPORT, MIGRATE, CREATESIP, CONNECTMANAGER };
    enum class NameRegistrationUIState { BLANK, INVALID, TAKEN, FREE, SEARCHING };
...
public:
    ...
    void setInputPara(QVariantMap para);
    QVariantMap getInputPara();
...
signals:
    ...
    void inputParaChanged(QVariantMap para);
