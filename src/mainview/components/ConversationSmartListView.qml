...
ListView {
    ...
    signal needToAccessMessageWebView(string currentUserDisplayName, string currentUserAlias, string currentUID, bool inCall)
    signal needToDeselectItems()
    signal needToBackToWelcomePage()

    signal currentIndexIsReset()
    signal modelIsSorted()

    function modelSorted() {
        conversationSmartListView.currentIndex = -1
        updateConversationSmartListView()
        conversationSmartListView.currentIndex = 0
    ...
    onCurrentIndexChanged: {
        if(conversationSmartListView.currentIndex === -1) {
            conversationSmartListView.currentIndexIsReset()
        }
    }
        ...
        MouseArea {
            ...
            onReleased: {
                ...
                if (mouse.button === Qt.RightButton && Qt.platform.os == "windows") {
                    ...
                    conversationSmartListView.needToAccessMessageWebView(DisplayID == DisplayName ? "" : DisplayID, DisplayName, UID, InCall)
                }
            }
            onEntered: {
                if(!InCall) {
