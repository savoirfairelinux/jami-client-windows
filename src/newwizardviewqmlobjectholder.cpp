...
void
NewWizardViewQmlObjectHolder::setInputPara(QMap<QString, QString> para)
{
    inputPara_ = para.toStdMap();
    emit imputParaChanged(para);
}

QMap<QString, QString>
NewWizardViewQmlObjectHolder::getInputPara()
{
    return QMap<QString, QString>(inputPara_);
}
...
{
    if (!fileDir.isEmpty()) {
        LRCInstance::accountModel().exportToFile(LRCInstance::getCurrAccId(),
                                                 fileDir + "/export.gz");
    }
    emit showMainViewWindow();
    emit LRCInstance::instance().accountListChanged();
...
Q_INVOKABLE void
NewWizardViewQmlObjectHolder::timeoutNameLookupTimer(const QString &nameServiceURL,
                                                     const QString &name)
{
