...
Window {
    ...
    function slotAccountListChanged(){
        var accountList = AccountAdapter.avModel().getAccountList()
        if(accountList.length === 0) {
            setSelected(SettingsView.Account)
        } else {
            currentAccountSettingsScrollWidget.disconnectAccountConnections()
        }
        var device = AccountAdapter.avModel().getDefaultDevice()
        if(device === ""){
            AccountAdapter.avModel().setCurrentVideoCaptureDevice(device)
        }
    }
