...
Rectangle {
    ...
    function populateVideoSettings() {
        ...
        if(devices.length === 0){
            deviceBox.append(qsTr("No Device"))
            deviceBox.currentIndex = 0

            formatBox.append(qsTr("No Device"))
            ...
            try{
                for(d of devices){
                    ...
                    deviceBox.append(UtilsAdapter.getStringUTF8(settings.name_qml))
                }
            } catch(err) {}
    ...
    property bool previewAvailable: AccountAdapter.isPreviewing()
