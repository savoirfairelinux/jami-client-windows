...
Rectangle {
    function populateAVSettings(){
        ...
        for(var id of inputDevices){
            inputComboBox.append({"textDisplay": UtilsAdapter.getStringUTF8(id), "firstArg": UtilsAdapter.getStringUTF8(id), "secondArg": ""})
        }
        ...
        for (var od of outputDevices) {
            outputComboBox.append({"textDisplay": UtilsAdapter.getStringUTF8(od), "firstArg": UtilsAdapter.getStringUTF8(od), "secondArg": ""})
        }
    ...
    function populateVideoSettings() {
        ...
        if(devices.length === 0){
            deviceBox.append({"textDisplay": qsTr("No Device"), "firstArg": "", "secondArg": ""})
            deviceBox.currentIndex = 0

            formatBox.append({"textDisplay": qsTr("No Device"), "firstArg": "", "secondArg": ""})
    ...
    function setFormatListForDevice(device){
        if(SettingsAdaptor.get_DeviceCapabilitiesSize(device) === 0) {
            return
        }
