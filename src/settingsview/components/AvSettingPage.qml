...
Rectangle {
    ...
    function populateAVSettings(){
        ...
        var inputDevices = AccountAdapter.avModel().getAudioInputDevices()
        ...
        var outputDevices = AccountAdapter.avModel().getAudioOutputDevices()
        ...
        var encodeAccel = AccountAdapter.avModel().getHardwareAcceleration()
    ...
    function populateVideoSettings() {
        ...
        deviceBox.enabled = (AccountAdapter.avModel().getDevices().length > 0)
        formatBox.enabled = (AccountAdapter.avModel().getDevices().length > 0)
        labelVideoDevice.enabled = (AccountAdapter.avModel().getDevices().length > 0)
        labelVideoFormat.enabled = (AccountAdapter.avModel().getDevices().length > 0)

        if(AccountAdapter.avModel().getDevices().length === 0){
            ...
            var currentCaptureDevice = AccountAdapter.avModel().getCurrentVideoCaptureDevice()
            // get last index of device vector
            var deviceLastIndex = AccountAdapter.avModel().getDevices().length - 1
            var devices = AccountAdapter.avModel().getDevices()
            try{
            ...
            // set format list
            setFormatListForDevice(AccountAdapter.avModel().getCurrentVideoCaptureDevice())
    ...
    function slotAudioOutputIndexChanged(index){
        ...
        AccountAdapter.avModel().setOutputDevice(selectedOutputDeviceName)
    ...
    function slotAudioInputIndexChanged(index){
        ...
        AccountAdapter.avModel().setInputDevice(selectedInputDeviceName)
        startAudioMeter(false)
    }
    ...
    function slotDeviceBoxCurrentIndexChanged(index){
        ...
        var devices = AccountAdapter.avModel().getDevices()
        try{
            var deviceId = AccountAdapter.avModel().getDeviceIdFromName(deviceName)
            if(deviceId.length === 0){
                console.warn("Couldn't find device: " + deviceName)
                return
            }

            AccountAdapter.avModel().setCurrentVideoCaptureDevice(deviceId)
            AccountAdapter.avModel().setDefaultDevice(deviceId)
    ...
    function slotFormatBoxCurrentIndexChanged(index){
        ...
        var device = AccountAdapter.avModel().getCurrentVideoCaptureDevice()
        try{
    ...
    Connections{
        target: AccountAdapter.getRenderManager()
