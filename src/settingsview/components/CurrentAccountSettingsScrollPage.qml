...
Rectangle {
    ...
    function verifyRegisteredNameSlot() {
        if (ClientWrapper.settingsAdaptor.get_CurrentAccountInfo_RegisteredName() !== "") {
            ...
            registeredName = ClientWrapper.utilsAdapter.stringSimplifier(
                        currentRegisteredID.text)
            if (registeredName !== "") {
                if (ClientWrapper.utilsAdapter.validateRegNameForm(registeredName)) {
    ...
    function setAccEnableSlot(state) {
        ClientWrapper.accountModel.setAccountEnabled(ClientWrapper.utilsAdapter.getCurrAccId(), state)
    }

    /*
     * FileDialog for exporting account
     */
    FileDialog {
        id: exportBtn_Dialog
        ...
        onAccepted: {
            ...
            if (ClientWrapper.accountAdaptor.hasPassword()) {
                ...
                if (exportPath.length > 0) {
                    var isSuccessful = ClientWrapper.accountAdaptor.accoundModel().exportToFile(ClientWrapper.utilsAdapter.getCurrAccId(), exportPath,"")
    ...
    DeleteAccountDialog{
        ...
        onAccepted: {
            ...
            if(ClientWrapper.utilsAdapter.getAccountListSize() > 0){
                navigateToMainView()
            }
        }
    }
    ...
    function updateAndShowBannedContactsSlot() {
        ...
        bannedListModel.layoutAboutToBeChanged()
        bannedListModel.dataChanged(bannedListModel.index(0, 0),
                                     bannedListModel.index(bannedListModel.rowCount() - 1, 0))
        bannedListModel.layoutChanged()
    }
    ...
    function updateAndShowDevicesSlot() {
        ...
        deviceItemListModel.layoutAboutToBeChanged()
        deviceItemListModel.dataChanged(deviceItemListModel.index(0, 0),
                                     deviceItemListModel.index(deviceItemListModel.rowCount() - 1, 0))
        deviceItemListModel.layoutChanged()
    }
