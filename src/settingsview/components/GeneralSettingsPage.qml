...
Rectangle {
    function populateGeneralSettings(){
        ...
        alwaysRecordingCheckBox.checked = AccountAdapter.avModel().getAlwaysRecord()
        recordPreviewCheckBox.checked = AccountAdapter.avModel().getRecordPreview()
        recordQualityValueLabel.text = UtilsAdapter.getRecordQualityString(AccountAdapter.avModel().getRecordQuality() / 100)
        recordQualitySlider.value = AccountAdapter.avModel().getRecordQuality() / 100

        AccountAdapter.avModel().setRecordPath(SettingsAdaptor.getDir_Document())
    ...
    function slotAlwaysRecordingClicked(state){
        AccountAdapter.avModel().setAlwaysRecord(state)
    }

    function slotRecordPreviewClicked(state){
        AccountAdapter.avModel().setRecordPreview(state)
    }
    ...
    function slotRecordQualitySliderSliderReleased(){
        ...
        AccountAdapter.avModel().setRecordQuality(value * 100)
    }
    ...
    onRecordPathChanged: {
        ...
        AccountAdapter.avModel().setRecordPath(recordPath)
    }
