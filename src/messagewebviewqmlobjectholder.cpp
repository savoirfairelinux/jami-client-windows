...
MessageWebViewQmlObjectHolder::setMessageWebViewQmlObject(QObject* obj)
{
    ...
    connect(messageWebViewQmlObject_, SIGNAL(sendMessage(const QString&)), this, SLOT(slotSendMessage(const QString&)));
    connect(messageWebViewQmlObject_, SIGNAL(sendImage(const QString&)), this, SLOT(slotSendImage(const QString&)));
    connect(messageWebViewQmlObject_, SIGNAL(sendFile(const QString&)), this, SLOT(slotSendFile(const QString&)));
    connect(messageWebViewQmlObject_, SIGNAL(setNewMessagesContent(const QString&)), this, SLOT(slotSetNewMessagesContent(const QString&)));
    connect(messageWebViewQmlObject_, SIGNAL(deleteInteraction(const QString&)), this, SLOT(slotDeleteInteraction(const QString&)));
}
...
MessageWebViewQmlObjectHolder::setupChatView(const QString& uid)
{
    ...
    Utils::oneShotConnect(messageWebViewQmlObject_, SIGNAL(sendMessageContentSaved(const QString&)), this, SLOT(slotSendMessageContentSaved(const QString&)));
...
void MessageWebViewQmlObjectHolder::slotSendFile(const QString& message)
{
    QFileInfo fi(message);
    QString fileName = fi.fileName();
    try {
...
MessageWebViewQmlObjectHolder::setMessagesImageContent(const QString& path, bool isBased64)
{
    if (isBased64) {
        ...
        QMetaObject::invokeMethod(messageWebViewQmlObject_, "webViewRunJavaScript", Q_ARG(QVariant, param));
    }
    else {
...
MessageWebViewQmlObjectHolder::setMessagesFileContent(const QString& path)
{
    ...
    QString param = QString("addFile_path('%1','%2','%3')").arg(path, fileName, Utils::humanFileSize(fileSize));
