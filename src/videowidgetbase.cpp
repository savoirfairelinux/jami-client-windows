                                             ...
                                             CUdeviceptr startPtr, CUcontext cudaContext)
{
    ...
    //Microsoft::WRL::ComPtr<cudaGraphicsResource> cudaResourceForCurrentTexture;
    //cudaGraphicsGLRegisterImage(cudaResourceForCurrentTexture.GetAddressOf(), texture->textureId(), GL_TEXTURE_2D, CU_GRAPHICS_REGISTER_FLAGS_NONE);
    //cuGraphicsResourceSetMapFlags(cudaResourceForCurrentTexture., CU_GRAPHICS_MAP_RESOURCE_FLAGS_WRITE_DISCARD);
    //cuGraphicsMapResources(1, cudaResourceForCurrentTexture.Get(), 0);
    //CUarray dstArray;
