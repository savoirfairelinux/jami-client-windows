...
D3DVideoWidgetBase::InitScene()
{
    ...
    //constantBufferDesc.ByteWidth = sizeof(ConstantBufferDataStruct) % 16 == 0 ? sizeof(ConstantBufferDataStruct) : 16 * (1 + (int)(sizeof(ConstantBufferDataStruct) / 16));
    ...
    ERROR_HANDLE_RUN(d3dDevice_->CreateBuffer(&constantBufferDesc, &constantBufferResourceData, constantBuffer_.GetAddressOf()), "The creation of constant buffer fails!");
