...
D3DVideoWidgetWindow::updateTextureFromCUDA(AVFrame* frame)
{
    ...
    try {
        AVHWFramesContext* avHWFramesContext = reinterpret_cast<AVHWFramesContext*>(frame->hw_frames_ctx);
        if (!avHWFramesContext || avHWFramesContext->device_ctx) return false;
        AVHWDeviceContext* device_ctx = avHWFramesContext->device_ctx;
        ...
        qDebug() << "Cannot parse cuda context successfully!";
        return result;
    }
    try {
