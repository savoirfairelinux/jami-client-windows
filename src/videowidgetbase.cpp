...
void
D3DVideoWidgetBase::initializeTextures(ID3D11Texture2D* texture, ID3D11ShaderResourceView* shaderResourceView,
                                       int width, int height, int index, D3D11_SUBRESOURCE_DATA resourceData)
{
    ...
    d3dDevice_->CreateTexture2D(&textureDesc, &resourceData, &texture);
    ...
    d3dDevice_->CreateShaderResourceView(texture, &shaderResourceViewDesc, &shaderResourceView);
}
...
D3DVideoWidgetBase::InitScene()
{
    ...
    ERROR_HANDLE_RUN(D3DCompileFromFile(L"shaders/ShaderJami.hlsl", 0, D3D_COMPILE_STANDARD_FILE_INCLUDE, "VS_main", "vs_4_0", 0, 0, &vertexShaderBlob_, &VS_ErrorMsg),"Vertex shader compile fails!");
    if(VS_ErrorMsg) {
        std::string error;
        error.assign((char*)VS_ErrorMsg->GetBufferPointer(), VS_ErrorMsg->GetBufferSize());
        qDebug() << QString::fromStdString(error);
    }
    ERROR_HANDLE_RUN(D3DCompileFromFile(L"shaders/ShaderJami.hlsl", 0, D3D_COMPILE_STANDARD_FILE_INCLUDE, "PS_main", "ps_4_0", 0, 0, &pixelShaderBlob_, &PS_ErrorMsg),"Pixel shader compile fails!");
    if (PS_ErrorMsg) {
        std::string error;
