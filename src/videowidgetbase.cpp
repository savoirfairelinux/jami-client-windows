...
D3DVideoWidgetWindow::initializeTextureWithCUDA(Microsoft::WRL::ComPtr<ID3D11Texture2D> texture,
                                                Microsoft::WRL::ComPtr<ID3D11ShaderResourceView> shaderResourceView,
                                                int width, int height, int lineSize, int index, CUdeviceptr starPtr, CUcontext cudaContext)
{
    D3D11_OBJECT_RELEASE(shaderResourceView);
    D3D11_OBJECT_RELEASE(texture);
    d3dDevContext_->Flush();
...
D3DVideoWidgetWindow::updateTextureFromCUDA(AVFrame* frame)
{
    ...
    AVHWFramesContext* avHWFramesContext = (AVHWFramesContext*)(frame->hw_frames_ctx);
    if (!avHWFramesContext || !avHWFramesContext->device_ref) return false;
    auto device_ctx =(AVHWDeviceContext*) avHWFramesContext->device_ref->data;
    if (!device_ctx) return false;
    AVCUDADeviceContext* avCUDADeviceContext = (AVCUDADeviceContext*)(device_ctx->hwctx);
    ...
    initializeTextureWithCUDA(texY_, shaderResourceViewY_, frame->width, frame->height, frame->linesize[0],
        0, pointerToDataBufferY, cudaContext);
    CUdeviceptr pointerToDataBufferU = (CUdeviceptr)frame->data[1];
    initializeTextureWithCUDA(texY_, shaderResourceViewY_, frame->width / 2, frame->height / 2, frame->linesize[1],
        1, pointerToDataBufferU, cudaContext);
    //CUdeviceptr pointerToDataBufferV = (CUdeviceptr)frame->data[2];
    //initializeTextureWithCUDA(texY_, shaderResourceViewY_, frame->width / 2, frame->height / 2, frame->linesize[2],
    //                          2, pointerToDataBufferV, cudaContext);
    result = true;
