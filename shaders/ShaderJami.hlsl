...
struct VS_Output {
    ...
    bool useD3D11HWAccel : USE_HW_D3D11;
};
...
VS_Output VS_main(VS_Input input){
    ...
    out_vtx.useD3D11HWAccel = isUsingD3D11HW;
...
Texture2D texD3DHW : register(t3);

float4 PS_main(VS_Output input) : SV_Target  {
    float3x3 convertMat = float3x3(1,1,1,
               0,-0.39465,2.03211,
               1.13983, -0.58060,  0);
    
    float3 yuv;
    float3 rgb;
    if(input.useD3D11HWAccel) {
        yuv.x = texD3DHW.Sample(samplerState,input.texCoorVSOut).r;
        yuv.y = texD3DHW.Sample(samplerState,input.texCoorVSOut).g - 0.5f;
        yuv.z = texD3DHW.Sample(samplerState,input.texCoorVSOut).b - 0.5f;
    } else {
