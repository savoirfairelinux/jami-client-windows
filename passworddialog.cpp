...
PasswordDialogQMLControllerObject::savePassword(QVariant currentPasswordEditPassword, QVariant passwordEditPassword)
{
    bool success = false;
    try {
        QFuture<bool> futureSavePassword = QtConcurrent::run(qobject_cast<PasswordDialog*>(parent()),&PasswordDialog::savePassword,currentPasswordEditPassword.toString(),passwordEditPassword.toString());
...
PasswordDialogQMLControllerObject::exportAccount(QVariant currentPasswordEditPassword)
{
    bool success = false;
    try {
        QFuture<bool> futureSavePassword = QtConcurrent::run(qobject_cast<PasswordDialog*>(parent()),&PasswordDialog::exportAccount,currentPasswordEditPassword.toString());
        futureSavePassword.waitForFinished();
        success = futureSavePassword.result();
    } catch (...) {
