    ...
    , ui(new Ui::VideoView)
{
    ...
    connect(LRCInstance::renderer(), &RenderManager::previewRenderingStarted,
        [this]() {
            ...
            if (previewImage) {
                previewWidget_->computeGeometry(previewImage);
                QPoint endPoint = getPreviewPosition(PreviewSnap::SE);
                previewWidget_->setGeometry(QRect(endPoint, previewImage->size()));
            }
        });
}
...
VideoView::mouseReleaseEvent(QMouseEvent* event)
{
    ...
    PreviewSnap previewLocation;
    if (previewCenter.x() >= videoWidgetCenter.x()) {
        if (previewCenter.y() >= videoWidgetCenter.y()) {
            previewLocation = PreviewSnap::SE;
        } else {
            previewLocation = PreviewSnap::NE;
        }
    } else {
        if (previewCenter.y() >= videoWidgetCenter.y()) {
            previewLocation = PreviewSnap::SW;
        } else {
            previewLocation = PreviewSnap::NW;
        }
    }
    QPoint endPoint = getPreviewPosition(previewLocation);
...
VideoView::mouseMoveEvent(QMouseEvent* event)
{
    ...
    // Disable tearing by not allowing VideoView repaint
    // until previewWidget is repainted.
