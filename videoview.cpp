...
VideoView::mouseMoveEvent(QMouseEvent* event)
{
    QRect& previewRect =  ui->videoWidget->getPreviewRect();
    if ( draggingPreview_ )
        if ( previewRect.left() > 0 && previewRect.top() > 0 && previewRect.right() < width() 
    ...
    if(resizingPreview_ and distance.dx() > minimalSize_ and distance.dy() > minimalSize_ and geometry().contains(event->pos()))
