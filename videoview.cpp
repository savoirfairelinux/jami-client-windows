...
VideoView::showContextMenu(const QPoint& pos)
{
    ...
    connect(shareAction, &QAction::triggered,
        [this]() {
            ...
            auto scalingRatio =  screen->logicalDotsPerInch() / 96;
            rect.setWidth(static_cast<int>(rect.width() * scalingRatio));
            rect.setHeight(static_cast<int>(rect.height() * scalingRatio));
#else
            rect.setSize(Utils::getRealSize(screen));
#endif
            LRCInstance::avModel().setDisplay(0, rect.x(), rect.y(), rect.width(), rect.height());
        });
