...
VideoView::pushRenderer(const std::string& callId, bool isSIP)
{
    ...
    QObject::disconnect(currentOverLayRendererPair_.second);
    ...
    currentOverLayRendererPair_.first->setTransferCallAndSIPPanelAvailability(isSIP);
    currentOverLayRendererPair_.first->callStarted(callId);
    currentOverLayRendererPair_.first->setVideoMuteVisibility(!LRCInstance::getCurrentCallModel()->getCall(callId).isAudioOnly);
...
VideoView::resetDistantRenderer(const std::string & callId)
{
    ...
    // reset parent incase deletion of distant renderer causes error
    previewRenderer_->setParent(nullptr);
    currentOverLayRendererPair_.first->lower();
    currentOverLayRendererPair_.second->lower();
    ...
    currentOverLayRendererPair_.second->raise();
