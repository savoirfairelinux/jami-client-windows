...
private:
    // new lrc
    const lrc::api::account::Info& currentAccount();
    void selectConversation(const QModelIndex& current, lrc::api::ConversationModel* convModel);
    bool setConvModel(lrc::api::ConversationModel* convModel);
    void reloadMessageModel(const std::string& convUid);
    void reloadSmartList();
