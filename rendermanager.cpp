    ...
    :avModel_(avModel)
{
    ...
    connect(previewFrameWrapper_.get(), &FrameWrapper::frameUpdated,
        [this](const std::string& id) {
            Q_UNUSED(id);
...
void
RenderManager::connectPreviewRendering()
{
...
RenderManager::addDistantRenderer(const std::string& id)
{
    ...
    auto fwIt = distantFrameWrapperMap_.find(id);
    if (fwIt != distantFrameWrapperMap_.end() && !fwIt->second->isRendering()) {
        // reconnect FrameWrapper to avmodel
        fwIt->second->connectRendering();
    } else {
        auto fw = std::make_unique<FrameWrapper>(avModel_, id);

        // connect this to the FrameWrapper
        distantConnectionMap_[id].started = connect(
            fw.get(), &FrameWrapper::renderingStarted,
            [this](const std::string& id) {
                qDebug() << id.c_str() << " distantRenderingStarted";
                emit distantRenderingStarted(id);
            });
        distantConnectionMap_[id].updated = connect(
            fw.get(), &FrameWrapper::frameUpdated,
            [this](const std::string& id) {
                qDebug() << id.c_str() << " distantFrameUpdated";
                emit distantFrameUpdated(id);
            });
        distantConnectionMap_[id].stopped = connect(
            fw.get(), &FrameWrapper::renderingStopped,
            [this](const std::string& id) {
                qDebug() << id.c_str() << " distantRenderingStopped";
        ...
        // add to map
        distantFrameWrapperMap_.insert(std::make_pair(id, std::move(fw)));
    }
}
...
{
    auto fwIt = distantFrameWrapperMap_.find(id);
    if (fwIt != distantFrameWrapperMap_.end()) {
        // disconnect FrameWrapper from this
        auto cIt = distantConnectionMap_.find(id);
        if (cIt != distantConnectionMap_.end()) {
            disconnect(cIt->second.started);
            disconnect(cIt->second.updated);
            disconnect(cIt->second.stopped);
        }

        // erase
        //fwIt->second.reset();
        distantFrameWrapperMap_.erase(fwIt);
    }
}
