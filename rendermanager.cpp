    ...
    :avModel_(avModel)
{
    ...
    connect(&avModel_, &lrc::api::AVModel::deviceEvent,
        [this] {
            auto currentDeviceName = avModel_.getDefaultDeviceName();
            // decide whether a device has plugged, unplugged, or nothing has changed
            auto currentDeviceListSize = avModel_.getDevices().size();
            VideoDeviceEvent deviceEvent = VideoDeviceEvent::Unchanged;
            if (currentDeviceListSize > deviceListSize_) {
                deviceEvent = VideoDeviceEvent::Plugged;
            } else if (currentDeviceListSize < deviceListSize_) {
                deviceEvent = VideoDeviceEvent::Unplugged;
            }
            ...
            if (currentDeviceListSize == 0) {
                ...
                if (deviceEvent == VideoDeviceEvent::Unplugged)
                    if (avModel_.getCurrentVideoCaptureDevice().empty()) {
                        avModel_.setCurrentVideoCaptureDevice(currentDeviceName);
                    }

            }
            emit videoDeviceListChanged(currentDeviceListSize);
