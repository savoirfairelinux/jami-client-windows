    ...
    case Button::accountSettingsButton:
        ...
        if (LRCInstance::getCurrentAccountInfo().profileInfo.type == lrc::api::profile::Type::SIP) {
            ...
            if (pastButton_ != Button::accountSettingsButton && previewed_) {
                ui->currentSIPAccountAvatar->setIsSettingPreviewed(previewed_);
            }
        } else {
...
void SettingsWidget::setConnections()
{
    ...
    connect(ui->currentSIPAccountAvatar, &PhotoboothWidget::settingWidgetPreviewTosettingWidgetPhotoBoothLeaveSignal, [this]() {
        this->disconnectPreviewRendering();
        this->connectStartedRenderingToPhotoBooth();
    });
}
...
void SettingsWidget::videoDeviceEventHandlerAndMediaSettingSetUp()
{
    ...
    std::pair<bool, bool> isSIPAndReinitial;
    if (LRCInstance::getCurrentAccountInfo().profileInfo.type == lrc::api::profile::Type::SIP) {
        if (ui->currentSIPAccountSettingsScrollWidget->isVisible() && ui->currentSIPAccountAvatar->isPhotoBoothOpened()) {
            isSIPAndReinitial = std::make_pair<bool, bool>(true, true);
        } else {
            isSIPAndReinitial = std::make_pair<bool, bool>(true, false);
        }
    } else {
        if (ui->currentAccountSettingsScrollWidget->isVisible() && ui->currentAccountAvatar->isPhotoBoothOpened()) {
            isSIPAndReinitial = std::make_pair<bool, bool>(false, true);
        } else {
            isSIPAndReinitial = std::make_pair<bool, bool>(false, false);
        }
    }
    ...
    if (hasVideoDevices) {
        ...
        if (plugStatus == Utils::devicePluggingType::Plugged && LRCInstance::avModel().getCurrentVideoCaptureDevice() == device && !deviceWasEmpty_) {
            ...
            isSIPAndReinitial.second = false;
        }
    ...
    if (isSIPAndReinitial.second) {
        // if we are in photo booth open state and in account setting widget,
        // reset the photo booth video
        if (isSIPAndReinitial.first)
            ui->currentSIPAccountAvatar->startBooth(true);
        else
            ui->currentAccountAvatar->startBooth(true);
        ...
        if (LRCInstance::getActiveCalls().size() && !ui->avSettings->isVisible())
            emit videoInputDeviceLoseConnectionSignal(Utils::videoWidgetSwapType::videoInputDeviceLoseConnection);
        startPreviewing(true);
    }
}
