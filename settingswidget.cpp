...
SettingsWidget::SettingsWidget(QWidget* parent)
    ...
    , gif(new QMovie(":/images/jami_rolling_spinner.gif"))
{
    ui->setupUi(this);
...
void SettingsWidget::setRegNameUi(RegName stat)
{
    ...
    switch (stat) {
    case RegName::BLANK:
        ui->currentRegisteredID->setStyleSheet("border-color: rgb(240, 240, 240);");
        regNameBtn_ = false;
    ...
    case RegName::SEARCHING:
        ui->currentRegisteredID->setStyleSheet("border-color: #2b5084;");
        regNameBtn_ = false;
    ...
    if (onlineVersion != QString(VERSION_STRING) && onlineVersion != "Null") {
        ...
        if (ret == QDialog::Accepted) {
            ...
            if (manager.getDownloadStatus() == 404) {
                QMessageBox::critical(0, "Download Statux", "Installer Download Failed, Please Contact Support");
                return;
            }
