...
void SettingsWidget::setAccEnableSlot(int state)
{
    ...
    if (confProps.accountPublish) {
        auto peermap = LRCInstance::peerDiscoveryModel().getNearbyPeers(QString::fromStdString(LRCInstance::getCurrAccId()));
        if (!peermap.empty())
...
{
    connect(LRCInstance::editablePeerDiscoveryModel(), &lrc::api::PeerDiscoveryModel::peerMapStatusChanged, this, &SettingsWidget::peermaptestslot);
    // btnExitSettings
