...
void SettingsWidget::populateAVSettings()
{
    ...
    connect(&LRCInstance::avModel(), &lrc::api::AVModel::audioMeter,
        [this](const std::string& id, float level) {
            if (id == "audiolayer_id") {
                /*auto convertedLevel = 113.0f + 20.0f * log10(level / 20.0f);
                ui->audioInputMeter->setValue(std::min(convertedLevel * 1.25f, 100.0f));*/
...
void SettingsWidget::startAudioMeter()
{
    LRCInstance::avModel().setAudioMeterState(true);
    ui->audioInputMeter->start();
    if (!LRCInstance::getActiveCalls().size()) {
        QtConcurrent::run([this] { LRCInstance::avModel().startAudioDevice(); });
    }
}

void SettingsWidget::stopAudioMeter()
{
    LRCInstance::avModel().setAudioMeterState(false);
    ui->audioInputMeter->stop();
    if (!LRCInstance::getActiveCalls().size()) {
        QtConcurrent::run([this] { LRCInstance::avModel().stopAudioDevice(); });
    }
}