...
class FrameWrapper : public QObject
{
    Q_OBJECT;
public:
    FrameWrapper(const std::string& id = video::PREVIEW_RENDERER_ID);
    ~FrameWrapper();

    void connectRendering();
    video::Renderer* getRenderer() { return renderer_;  }
    video::Frame getFrame() { return frame_; }
...
public:
    RenderManager();
    ~RenderManager();
    video::Renderer* getPreviewRenderer() { return previewFrameWrapper_->getRenderer(); }
    video::Frame getPreviewFrame() { return previewFrameWrapper_->getFrame(); }
...
public:
    ...
    static RenderManager* getRenderDistributer() {
    ...
    static void reset(bool newInstance = false) {
        if (newInstance) {
            instance().renderer_.reset(new RenderManager());
...
private:
    ...
    LRCInstance(migrateCallback willMigrateCb = {},
        ...
        renderer_ = std::make_unique<RenderManager>();
    };
