...
struct VS_Output {
    ...
    float boarderRadius : BOARDER_RADIUS;
...
VS_Output VS_main(VS_Input input){
    ...
    out_vtx.boarderRadius = roundedCornerRadius;
...
// bool isDotInCircleUnormalizedCoordinate(float radinusUnNormalized, float2 dotNormalized, float2 centerNormalized,
//                                         float2 textureSize, );
...
float4 PS_main(VS_Output input) : SV_Target  {
    ...
    float radius = input.boarderRadius;
    ...
    if((texCor.x > radius && texCor.x < (1 - radius)) || (texCor.y > radius && texCor.y < (1 - radius))) {
        out_px = float4(rgb,1.0f);
    } else if(isDotInsideCircle(texCor,CenterA,radius)){
        out_px = float4(rgb,1.0f);
    } else if(isDotInsideCircle(texCor,CenterB,radius)){
        out_px = float4(rgb,1.0f);
    } else if(isDotInsideCircle(texCor,CenterC,radius)){
        out_px = float4(rgb,1.0f);
    } else if(isDotInsideCircle(texCor,CenterD,radius)){
        out_px = float4(rgb,1.0f);
    } else {
