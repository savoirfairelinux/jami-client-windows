...
CallAudioOnlyAvatarOverlay::writeAvatarOverlay(const std::string& accountId, const lrc::api::conversation::Info& convInfo)
{
    ...
    avatarPix_.loadFromData(QByteArray::fromBase64(contact.profileInfo.avatar.c_str()));
    avatarImg_ = avatarPix_.toImage();
    int size = min(avatarImg_.width(), avatarPix_.height());
    QImage CircledAvatar = Utils::getCirclePhoto(avatarImg_, size);
