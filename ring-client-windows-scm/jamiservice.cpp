...
{
    DWORD id;
    writeEventLogEntry(L"JamiService is started", EVENTLOG_INFORMATION_TYPE);
...
{
    writeEventLogEntry(L"JamiService is stopped", EVENTLOG_INFORMATION_TYPE);
...
JamiProcessMonitor::serviceWorkerThread(void* param)
{
    ...
    while (!currentInstance->stopping_) {
        currentInstance->writeEventLogEntry(L"JamiService is is laughing at you!", EVENTLOG_INFORMATION_TYPE);
        Beep(450, 150);
        Sleep(4000);
    }
